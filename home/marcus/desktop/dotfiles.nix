# The UI-managed configs on the desktop, both directions: symlinked out
# of the store into the shared ../common/dotfiles/ so edits made from
# inside Zed / the IDEs land in the repo as ordinary git drift, and that
# drift committed + pushed at activation (pathspec-scoped so unrelated
# dirty work never rides along; activation never fails over git; a failed
# push warns and leaves the commit). Same hook as the mac's — niri.kdl,
# dms.settings.json and xremap.yml are linked by desktop/niri.nix but
# live under the same pathspec, so their drift rides this hook too.
#
    ...
    if [ -d "${repo}/.git" ]; then
      ...
      if ! ${pkgs.git}/bin/git -C "${repo}" diff --cached --quiet -- ${pathArgs}; then
        ...
        if run ${pkgs.git}/bin/git -C "${repo}" commit -q \
          -m "chore(config): sync ui-managed files from the laptop" -- ${pathArgs}; then
