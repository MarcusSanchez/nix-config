...
modules/common/            default.nix packages.nix (cross-platform CLIs
  ...
  secrets.nix              also carries the super tier: fly_token is
                           declared only for hostnames in its `trusted`
                           list, which must track .sops.yaml's
                           recipients and key reality (the header says
                           why a mismatch loses everything)
